export class String {
  ...
  static formatPhoneWithCode(phoneNumber: string, phoneCode: string) {
    ...
    return `(+${phoneNumber.slice(
      indexOfPhoneCode,
      indexOfPhoneCode + phoneCode.length
    )}) ${this.addSpaces(phone, 3)}`;
  }
  ...
  static beautyHTML = (str: string) => {
    ...
    return str
      .replace(urlRegex, (url) => '<a href="' + url + '" target="__blank">' + url + "</a>")
  ...
  static compact(string: any, firstPart: number, lastPart: number, replaceWith = "...") {
    if (!string) return "";
    return string.replace(`${string}`.slice(firstPart, string.length - lastPart), replaceWith);
  }
  ...
  static extractFromHTML(html?: string, space?: boolean) {
    ...
    if (space) {
      ...
      for (var i = 0; i < children.length; i++) {
        ...
        else if ((children[i] as any).innerText) (children[i] as any).innerText += " ";
      }
    }
...
export const getAvatarInitials = (name: string) => {
  try {
    ...
    const _name = words.length > 1 ? `${words[0][0] + words[1][0]}` : words[0].slice(0, 2).trim();

    return String.limitCharacters(String.toSlug(_name).toUpperCase(), 2, "");
  } catch (error) {
    return "";
  }
};
