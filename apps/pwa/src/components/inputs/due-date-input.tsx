...
export const DueDateInput: FC<DueDateInputProps> = (props) => {
  ...
  const onChangePointedValue = (date: Date | number, applyPrevTime = true) => {
    ...
    if (pointed === "start") {
      props.onChange?.({
        startDate: DateTimeUtils.timeToSeconds(temp),
        dueDate: props.dueDate,
      });
    } else {
      props.onChange?.({
        startDate: props.startDate,
        dueDate: DateTimeUtils.timeToSeconds(temp),
      });
    }
  };
        ...
        <Group align="start">
          <Stack flex={1} gap={10} py={10} bg={configs.backgroundColors[colorScheme]} mb={-10} ml={-10} mr={-16} px={5}>
            <Suggestion
              ...
              onSelect={() => {
                onChangePointedValue(DateTimeUtils.getStartEndOfDay(new Date()).end);
              }}
            />
            ...
            <Suggestion
              ...
              value={renderDate(DateTimeUtils.getStartEndOfWeek(Date.now()).end)}
              onSelect={() => {
                onChangePointedValue(DateTimeUtils.getStartEndOfWeek(Date.now()).end);
              }}
            />
