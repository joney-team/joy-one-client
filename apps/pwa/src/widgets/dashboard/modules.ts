...
import {
  IconBusinessplan, IconCalendar, IconCashRegister,
  IconChartLine, IconNotes, IconStack2, IconUsersPlus
...
export const dashboardWidgetModules: EWidgetModules<DashboardWidgetType, DashboardWidgetsContext> = {
  [DashboardWidgetType.TODAY_REVENUE]: {
    ...
    component: numberWidget({
      ...
      renderSparkline: (ctx) => ctx.rangeReports.data?.period.map(r => r.data.receipts.revenue) || [],
      onClick: (ctx) => ctx.router.push(`/reports`, {
        'period': Period.DATE,
        'date': DateTimeUtils.timeToSeconds(),
      }),
      boxColor: (ctx) => {
        const totalRevenue = ctx.realtimeReport.data?.data.receipts.revenueToday || 0;
        return totalRevenue >= 0 ? 'primary' : 'red';
      },
      tooltip: (ctx) => ctx.realtimeReport.data?.updatedAt
  ...
  [DashboardWidgetType.TODAY_NEW_CUSTOMERS]: {
    ...
    component: numberWidget({
      ...
      renderSparkline: (ctx) => ctx.rangeReports.data?.period.map(r => r.data.customers.total) || [],
      onClick: (ctx) => ctx.router.push(`/customers`, {
        'cus-timeRangeCreatedAt': `${Period.DATE}-${DateTimeUtils.timeToSeconds()}`,
      }),
    })
  },
  [DashboardWidgetType.TODAY_BOOKINGS]: {
    ...
    component: numberWidget({
      ...
      onClick: (ctx) => ctx.router.push(`/bookings`, {
        'bk-view': 'day',
        'bk-date': `${DateTimeUtils.timeToSeconds()}`,
      }),
    })
  },
  [DashboardWidgetType.REVENUE_CHART]: {
    ...
    component: chartWidget({
      ...
      renderData: (ctx) => ctx.rangeReports.data?.period.map((v, i) => {
        const date = DateTimeUtils.secondsToTime(v.data.fromTime);
        return ({
          date: date ? date.getDate() : '-',
          value: reportConvertMoneyAmount(v.data.receipts.revenue, ctx.workspace.currency),
          prevValue: reportConvertMoneyAmount(ctx.rangeReports.data?.prevPeriod[i]?.data.receipts.revenue || 0, ctx.workspace.currency),
        })
      }),
  ...
  [DashboardWidgetType.LOANS_ACTIVATED_CONTRACTS]: {
    ...
    component: numberWidget({
      ...
      onClick: (ctx) => ctx.router.push(`/loans`, {
        'ltab': 'active',
      }),
      tooltip: (ctx) => ctx.realtimeReport.data?.updatedAt
        ? t('last_updated_at', { dateTime: renderDateTime(ctx.realtimeReport.data?.updatedAt) })
  ...
  [DashboardWidgetType.LOANS_FULFILLED_AMOUNT_CHART]: {
    ...
    component: chartWidget({
      ...
      renderData: (ctx) => ctx.rangeReports.data?.period.map((v, i) => {
        ...
        return ({
          date: date ? date.getDate() : '-',
          value: reportConvertMoneyAmount(v.data.loans.contracts.fulfilledAmount || 0, ctx.workspace.currency),
          prevValue: reportConvertMoneyAmount(ctx.rangeReports.data?.prevPeriod[i]?.data.loans.contracts.fulfilledAmount || 0, ctx.workspace.currency),
        })
      }),
      unit: (ctx) => reportConvertMoneyAmountUnit(ctx.workspace.currency),
      renderSeries: () => [
  ...
  [DashboardWidgetType.TASKS_COMPLETED_RATE_CHART]: {
    ...
    component: chartWidget({
      ...
      renderData: (ctx) => ctx.rangeReports.data?.period.map((v, i) => {
        ...
        return ({
          date: date ? date.getDate() : '-',
          value: v.data.tasks.total > 0 ? round(v.data.tasks.completed * 100 / v.data.tasks.total, 1) : 0,
          prevValue: ctx.rangeReports.data?.prevPeriod[i] && ctx.rangeReports.data?.prevPeriod[i].data.tasks.total > 0 ? round(ctx.rangeReports.data?.prevPeriod[i].data.tasks.completed * 100 / ctx.rangeReports.data?.prevPeriod[i].data.tasks.total, 1) : 0,
        })
      }),
      renderSeries: () => [
        { name: 'prevValue', label: t('last_month'), color: 'gray.4', strokeDasharray: '2 2' },
