...
export const fileTypes: Record<FileType, { label: () => string; icon: Icon }> = {
  ...
  [FileType.PDF]: { label: () => t`PDF`, icon: IconPdf },
  [FileType.MS_WORD]: { label: () => t`MS Word`, icon: IconFile },
  [FileType.MS_EXCEL]: { label: () => t`MS Excel`, icon: IconFileExcel },
  [FileType.MS_POWERPOINT]: { label: () => t`MS PowerPoint`, icon: IconPresentationAnalytics },
