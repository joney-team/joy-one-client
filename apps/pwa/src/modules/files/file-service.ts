...
export function getMineTypeAccept(fileType: FileType[]) {
  ...
  if (fileType.includes(FileType.VIDEO)) output = [
    ...output,
    'video/mp4',
    'video/x-msvideo',
    'video/mpeg',
...
export async function uploadFile(uploadFile: UploadFile) {
  ...
  if (isImage && (uploadFile.compressSize !== 0 && file.size / (1024 * 1024) > imgCompressSize) || uploadFile.maxWidthOrHeight) {
    file = new File(
      [await imageCompression(uploadFile.file, uploadFile.maxWidthOrHeight ? { maxWidthOrHeight: uploadFile.maxWidthOrHeight }
  ...
  if (uploadFile.relatedCustomerId) formData.append('relatedCustomerId', uploadFile.relatedCustomerId);
  ...
  if (uploadFile.relatedHrmTimekeepingId) formData.append('relatedHrmTimekeepingId', uploadFile.relatedHrmTimekeepingId);
  ...
  return apiTools.formData<FileEntity>('/files/upload', formData)
}
...
export async function onUploadFiles(files: UploadFile[], onUploaded?: (files: FileEntity[]) => Promise<void> | void) {
  return onActionLoad<FileEntity[]>({
    ...
    process: async () => {
      ...
      if (onUploaded) await onUploaded(_files);
      return _files;
    }
  })
}

export async function onUploadFile(file: UploadFile, onUploaded?: (file: FileEntity) => Promise<void> | void) {
  return onActionLoad<FileEntity>({
    name: t('upload_files'),
    icon: IconUpload,
    process: async () => {
