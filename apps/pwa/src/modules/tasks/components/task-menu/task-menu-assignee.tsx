...
export const TaskMenuAssignee: TaskMenuComponent = ({ task, groupVariables, updateTask }) => {
  ...
  const onFetchMore = useCallback(async () => {
    ...
    await fetchMore({
      variables: { ignoreSelf: true, offset: data.list.results.length },
    ...
    <Stack gap={0} align="stretch" miw={0}>
      ...
      <ScrollArea.Autosize mah={220} offsetScrollbars scrollbarSize={6} viewportRef={scrollRef}>
        <Stack px={5} gap={0}>
          {textSearch.length === 0 && (
            <Fragment>
              {member && (
                <MenuItem
                  member={member}
                  isSelected={selected.some((u) => u._id === member.userId)}
                  onClick={() => {
                    const isSelected = selected.some((u) => u._id === member.userId);
                    const data = isSelected
                      ? selected.filter((t) => t._id !== member.userId)
                      : [...selected, member];
              ...
              {selected.map((selectedMember) => {
                const isSelf = selectedMember.userId === member?.userId;
                ...
                return (
                  <MenuItem
                    ...
                    isSelected
                    onClick={() => {
                      const data = selected.filter((t) => t._id !== selectedMember.userId);
                      setSelected(data);
          ...
          {!isSearchEmpty &&
            data?.list.results.map((member) => {
              const isSelected = selected.some((u) => u.userId === member.userId);
              ...
              return (
                <MenuItem
                  ...
                  onClick={() => {
                    const data = isSelected
                      ? selected.filter((t) => t.userId !== member.userId)
                      : [...selected, member];
