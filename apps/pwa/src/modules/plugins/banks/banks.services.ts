...
import { BankAccount, BankInformation, BankQrCode } from "./banks.types";
...
export async function getBanks(): Promise<ResponseList<BankInformation>> {
  if (banks.length) return { data: banks, count: banks.length };
  try {
    const data = await MainRequest.get(`/plugins/banks`)
    banks = data.data;
    return data;
  } catch (error) {
...
export async function getQrCodePaymentUrl(bankAccount: BankAccount, payload: { amount: number, description?: string }) {
...
export async function getQrCode(bankAccount: BankAccount, payload: { amount: number, description?: string }): Promise<BankQrCode> {
  const banks = await getBanks();
  const bank = banks.data.find(v => v.id === bankAccount.bankId);
...
export function getStaticQrCode(bank: BankInformation, bankAccount: BankAccount, payload: {
