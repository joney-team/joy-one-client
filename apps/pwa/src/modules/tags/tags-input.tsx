...
export const TagsInput: FC<TagsInputProps> = (props) => {
  const tags = useTags();
  const value = props.value?.map((v) => tags.list.find((t) => t._id === v)).filter((v) => !!v) as TagEntity[];

  const toogleSelect = (tag?: TagEntity) => {
    if (!tag || props.disabled) return;
    const tags = value.find((t) => t._id === tag._id) ? value.filter((t) => t._id !== tag._id) : [...value, tag];
