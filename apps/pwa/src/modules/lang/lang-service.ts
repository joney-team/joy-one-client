...
export const num = (value: any, args?: { roundPrecision?: number, type?: 'money' | 'hours', suffix?: string, prefix?: string, empty?: string }): string => {
  let _args = { ...args };
  ...
  const render = () => {
    if (!value || typeof value !== 'number' || Number.isNaN(+value) || +value === 0) return args?.empty || "0";
    let _value = value;
...
export const renderRangeTime = (from: Date | number, to: Date | number) => {
  ...
  if (state.isTwelveHour && fromDate.format('a') === toDate.format('a')) return `${fromDate.format('hh:mm')} - ${toDate.format('hh:mm A')}`;
