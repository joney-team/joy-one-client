...
import { ModalPayReceipt, ModalPayReceiptProps } from "@/modules/receipts/modals/modal-pay-receipt";
...
export const ModalLoanLiquidation: FC<LoanEntity> = (loan) => {
  ...
  const onSubmit = async (onPay: (args: ModalPayReceiptProps) => void) => {
      ...
      {(onPayReceipt) => (
        <Stack>
                    ...
                    {loan.paymentPeriods && (
                      <small>
                        ...
                        {loan.paymentPeriods.find((v) => v.period === 1)?.startTime && (
                          <DateFormat
                            value={
                              loan.paymentPeriods.find((v) => v.period === 1)?.startTime as number
                            }
                            type="date"
                          />
                        )}{" "}
          ...
          <Text ta="center" c="orange">
            <Trans>
              Are you sure you want to liquidate this loan? This action cannot be undone.
            </Trans>
          </Text>
