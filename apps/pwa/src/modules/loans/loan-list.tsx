...
import { ModalCreateLoan } from "./modals/modal-create-loan";
            ...
            <List<LoanEntity>
                  ...
                  render: ({ data: loan }) => {
                    ...
                    const linkContractPdf =
                      ...
                      !!workspace.settings.loanSettings?.contractPdfUrl
                        ? workspace.settings.loanSettings?.contractPdfUrl?.replace(
                            "{code}",
                            loan.code
                          )
                        : undefined;
                  ...
                  render: ({ value, data: loan }) => {
                    ...
                    return (
                      <Stack gap={3}>
                        ...
                        >
                          {loan.nextReceiptAt && (
                            <DateFormat value={loan.nextReceiptAt} type="date" />
                          )}
                        </Text>

                        <Text fz={10} c={isExpired ? "red" : isWarning ? "orange" : "gray"}>
                          {loan.nextReceiptAt && (
                            <RelativeTimeFormat value={loan.nextReceiptAt - diff} />
                          )}
                        </Text>
                      </Stack>
                    );
              ...
              bulkActions={[
                ...
                },
                {
                  ...
                  available: (data) =>
                    data.every((v) =>
                      [LoanStatus.PENDING, LoanStatus.PENDING_SIGN].includes(v.status)
                    ),
