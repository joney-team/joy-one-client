...
import type { WorkspaceView, WorkspaceViewComponent } from "../workspace-settings/workspace-settings-types";
...
export const getDefaultWorkspaceView = (type?: WorkspaceType): WorkspaceView => {
  ...
  return {
    ...
    reportWidgets: view.reportWidgets?.map((v, i) => ({
      ...v,
      id: i.toString()
    })) ?? null,
    dashboardWidgets: view.dashboardWidgets?.map((v, i) => ({
      ...v,
      id: i.toString()
    })) ?? null,
  };
