...
export class DateTime {
  static isValid(raw: RawDate): boolean {
    return (
      this.normalizeDate(raw) instanceof Date &&
      !isNaN(this.normalizeDate(raw).getTime())
    );
  }
  ...
  static isSeconds(
    ...
    const maxTimestamp = Math.floor(
      new Date(maxYear, 11, 31, 23, 59, 59).getTime() / 1000,
    );
  ...
  static format(
    date: RawDate,
    format?: Intl.DateTimeFormatOptions & { locale?: string },
  ) {
  ...
  static add(date: RawDate, unit: DateTimeUnit, amount: number) {
    ...
    if (safeUnit === 'week') {
      return new Date(
        normalizedDate.setDate(normalizedDate.getDate() + amount * 7),
      );
    }

    if (safeUnit === 'month') {
      return new Date(
        normalizedDate.setMonth(normalizedDate.getMonth() + amount),
      );
    }

    if (safeUnit === 'year') {
      return new Date(
        normalizedDate.setFullYear(normalizedDate.getFullYear() + amount),
      );
    }
  ...
  static diff(date: RawDate, compareDate: RawDate, unit: DateTimeUnit) {
    ...
    if (safeUnit === 'week') {
      ...
      return Math.floor(
        Math.abs(date1.getTime() - date2.getTime()) / (1000 * 60 * 60 * 24 * 7),
      );
    }
