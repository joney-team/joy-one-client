...
import { Anchor, Box, Button, Center, Container, Grid, Group, Image, Stack, Text, ThemeIcon, Title, rem } from "@mantine/core";
            ...
            <Grid pt={isMobile ? 30 : 70} gutter={80}>
              <Grid.Col span={{ md: 12 }}>
                <Stack gap={30} align="center">
                  <Title ta="center" fz={rem(isMobile ? 45 : 55)} c="primary">ENJOY <span style={{ fontWeight: 200 }}>WORK {isMobile ? <br /> : ''} IN </span> ONE <span style={{ fontWeight: 200 }}>APP</span></Title>

                  <Stack gap={3}>
                    <Text ta="center" fz={rem(isMobile ? 15 : 22)}>{t('intro_title')}</Text>
                  </Stack>
      ...
      <Container size="lg" py={100}>
        <Stack>
          <SessionTitle
            ...
            description={<Text fz={rem(20)} ta="center">
              {t('session_one_desc')}
            </Text>}
          />
              ...
              <Grid align="center">
                ...
                <Grid.Col span={{ md: 5 }} p={30}>
                  <Group>
                    ...
                    <Title order={2} fw={300}>{t('task_management')}</Title>
                  </Group>
                  <Stack gap={15} mt={20}>
                    <Text>
                      {t('task_management_desc_1')}
                    </Text>
                    <Text>
                      {t('task_management_desc_2')}
                    </Text>
                    <Text>
                      {t('task_management_desc_3')}
                    </Text>
                    <Text>
                      {t('task_management_desc_4')}
                    </Text>
                  </Stack>
                </Grid.Col>
              </Grid>
              ...
              <Grid align="center">
                <Grid.Col span={{ md: 5 }} p={30}>
                  <Group>
                    ...
                    <Title order={2} fw={300}>{t('reports')}</Title>
                  </Group>
                  <Stack gap={15} mt={20}>
                    <Text>
                      {t('reports_desc_1')}
              ...
              <Grid align="center">
                ...
                <Grid.Col span={{ md: 5 }} p={30}>
                  <Group>
                    ...
                    <Title order={2} fw={300}>{t('messages')}</Title>
                  </Group>
                  <Stack gap={15} mt={20}>
                    <Text>
                      {t('messages_desc_1')}
...
const SessionTitle: FC<{ title: any, description?: React.ReactNode }> = (props) => {
  return <Stack align="center" mb={50} w="100%">
    <Title c="primary" fw={200} fz={rem(40)} tt="uppercase" ta="center">{props.title}</Title>
    {props.description && <Center w="100%">
      <Box style={{ maxWidth: "100%", width: 650 }}>
        {props.description}
      </Box>
    </Center>}
  </Stack>
...
const BenefitSession: FC<{
  ...
  return <Stack gap={40}>
    ...
    <Stack gap={10}>
      <Title c="primary" ta="center" order={2}>{props.title}</Title>
      {props.benefits.map((benefit, index) => <Text ta="center" key={index}>{benefit}</Text>)}
    </Stack>
  </Stack>
}