...
export class ApiInstance {
  ...
  constructor(options?: ApiInstanceOptions) {
    ...
    instance.interceptors.response.use((res) => res, async (error) => {
      ...
      if (error.response?.status === 401 && !originalRequest._retried && !isRequestRefreshToken && currentAccessToken) {
  ...
  async post<Response = any, Payload = any>(url: string, payload?: Payload, config?: AxiosRequestConfig) {
    return this.instance.post<Response>(url, payload, await this.bindConfig(config ?? {}))
      .then((res) => res.data);
  }

  async put<Response = any, Payload = any>(url: string, payload?: Payload, config?: AxiosRequestConfig) {
    return this.instance.put<Response>(url, payload, await this.bindConfig(config ?? {}))
      .then((res) => res.data);
  }

  async patch<Response = any, Payload = any>(url: string, payload?: Payload, config?: AxiosRequestConfig) {
    return this.instance.patch<Response>(url, payload, await this.bindConfig(config ?? {}))
      .then((res) => res.data);
  }
